...
describe("tool_call_diff", function()
    ...
    describe("extract_diff_blocks", function()
        it("creates new file block when old_text is nil", function()
            ...
            local first_block = assert.not_nil(blocks[1])
            assert.equal(1, first_block.start_line)
            assert.equal(0, first_block.end_line)
            assert.equal(0, #first_block.old_lines)
            assert.equal(3, #first_block.new_lines)
        end)
        ...
        it(
            ...
            function()
                ...
                assert.equal(1, #blocks)
                local first_block = assert.not_nil(blocks[1])
                assert.same({ "line 1." }, first_block.old_lines)
                assert.same({ "line 1.", "line 2." }, first_block.new_lines)
            end
        )
        ...
        it("handles single line modification via substring match", function()
            ...
            assert.equal(1, #blocks)
            local first_block = assert.not_nil(blocks[1])
            assert.equal(2, first_block.start_line)
            assert.equal(2, first_block.end_line)
            assert.same({ "const y = 2;" }, first_block.old_lines)
            assert.same({ "const y = 42;" }, first_block.new_lines)
        end)
        ...
        it("finds all occurrences when replace_all is true", function()
            ...
            local first_block = assert.not_nil(blocks[1])
            local second_block = assert.not_nil(blocks[2])
            assert.equal(1, first_block.start_line)
            assert.equal(3, second_block.start_line)
        end)
        ...
        it(
            ...
            function()
                ...
                assert.equal(1, #blocks)
                local block = assert.not_nil(blocks[1])
    ...
    describe("filter_unchanged_lines", function()
        it("filters unchanged lines and preserves indices", function()
            ...
            local first_pair = assert.not_nil(filtered.pairs[1])
            local second_pair = assert.not_nil(filtered.pairs[2])
            assert.equal(2, first_pair.old_idx)
            assert.equal(5, second_pair.old_idx)
        end)
        ...
        it("handles pure insertions (empty old)", function()
            ...
            assert.equal(2, #filtered.pairs)
            local first_pair = assert.not_nil(filtered.pairs[1])
            assert.is_nil(first_pair.old_line)
        end)
        ...
        it("handles pure deletions (empty new)", function()
            ...
            assert.equal(2, #filtered.pairs)
            local first_pair = assert.not_nil(filtered.pairs[1])
            assert.is_nil(first_pair.new_line)
        end)
        ...
        it("handles insertion with unchanged context line", function()
            ...
            local first_pair = assert.not_nil(filtered.pairs[1])
            assert.is_nil(first_pair.old_line)
            assert.equal("test=true", first_pair.new_line)
        end)
    end)
    ...
    describe("minimize_diff_blocks", function()
        ...
        it("keeps changed single-line block as-is", function()
            ...
            local first_block = assert.not_nil(minimized[1])
            assert.same({ "old" }, first_block.old_lines)
            assert.same({ "new" }, first_block.new_lines)
        end)
        ...
        it("keeps insertion with unchanged context", function()
            ...
            assert.equal(1, #minimized)
            local first_block = assert.not_nil(minimized[1])
            assert.same({ "ignore-scripts=false" }, first_block.old_lines)
            assert.same(
                { "ignore-scripts=false", "test=true" },
                first_block.new_lines
            )
        end)
        ...
        it("extracts only changed lines from multi-line block", function()
            ...
            local first_block = assert.not_nil(minimized[1])
            local second_block = assert.not_nil(minimized[2])
            assert.same({ "old2" }, first_block.old_lines)
            assert.same({ "new2" }, first_block.new_lines)
            assert.same({ "old4" }, second_block.old_lines)
            assert.same({ "new4" }, second_block.new_lines)
        end)
        ...
        it("handles pure insertion and pure deletion", function()
            ...
            local first_insertion = assert.not_nil(min_ins[1])
            assert.equal(0, #first_insertion.old_lines)
            assert.same({ "new line" }, first_insertion.new_lines)

            local min_del = ToolCallDiff.minimize_diff_blocks(deletion)
            assert.equal(1, #min_del)
            local first_deletion = assert.not_nil(min_del[1])
            assert.same({ "deleted" }, first_deletion.old_lines)
            assert.equal(0, #first_deletion.new_lines)
        end)
    end)
end)
